...
-- Division of two numbers (returns a decimal result)
SELECT (5/2) AS DIVIDE;

-- Modulus operation (returns the remainder of division)
SELECT (5%2) AS MODULUS;

-- Fetch all staff types (may contain duplicates)
SELECT STAFF_TYPE FROM STAFF;

-- Fetch distinct staff types (eliminates duplicates)
SELECT DISTINCT STAFF_TYPE FROM STAFF;

-- Fetch the first 5 records of staff types
...
SELECT *
FROM STUDENTS
WHERE DOB = STR_TO_DATE('13-JAN-2014', '%d-%b-%Y')
LIMIT 10000;
...
FROM TABLE1 T1
JOIN TABLE2 T2 ON T1.C1 = T2.C1 AND T1.C2 = T2.C2;
...
-- Fetch all the class names where Music is taught as a subject
SELECT CLASS_NAME
FROM SUBJECTS SUB
...
SELECT DISTINCT CONCAT(STF.FIRST_NAME, ' ', STF.LAST_NAME) AS FULL_NAME
FROM SUBJECTS SUB
...
SELECT STF.STAFF_TYPE, CONCAT(STF.FIRST_NAME, ' ', STF.LAST_NAME) AS FULL_NAME, STF.AGE,
       CASE WHEN STF.GENDER = 'M' THEN 'Male'
            WHEN STF.GENDER = 'F' THEN 'Female'
       END AS GENDER, STF.JOIN_DATE
FROM STAFF STF
...
UNION ALL
SELECT STAFF_TYPE, CONCAT(FIRST_NAME, ' ', LAST_NAME) AS FULL_NAME, AGE,
...
SELECT SC.CLASS_ID, COUNT(*) AS no_of_students
...
--  Subquery: Fetch the details of parents having more than 1 kid going to this school. Also display student details
...
-- Staff details whose salary is less than 5000
